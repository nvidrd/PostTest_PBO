package PostTest_6;


import PostTest_5.Karyawan;

public class Karyawan_Magang extends Karyawan implements Karyawan_Interface{
    private int id_magang;
    private String masa_magang;
    private final String role = "Karyawan Magang";

    public Karyawan_Magang(String Nama, String Nohp, String Jabatan, String Alamat, int ID_magang, String Masa_Magang, String role){
        super(Nama, Nohp, Jabatan, Alamat);
        this.setId_magang(ID_magang);
        this.setMasa_magang(Masa_Magang);
    }

    public int getId_magang() {
        return id_magang;
    }

    public void setId_magang(int id_magang) {
        this.id_magang = id_magang;
    }

    public String getMasa_magang() {
        return masa_magang;
    }

    public void setMasa_magang(String masa_magang) {
        this.masa_magang = masa_magang;
    }


    // IMPLEMENTS INTERFACE
    @Override
    public void sukses_add(){
        System.out.println("\n-----------------------------------------------");
        System.out.println("|              DATA DITAMBAHKAN               |");
        System.out.println("-----------------------------------------------");
        System.out.println("ID Karyawan            : KM" + this.getId_magang());
        System.out.println("Nama Karyawan          : " + this.getNama());
        System.out.println("No. HP Karyawan        : " + this.getNohp());
        System.out.println("Jabatan Karyawan       : " + this.getJabatan());
        System.out.println("Alamat Karyawan        : " + this.getAlamat());
    }

    @Override
    public void sukses_upd(){
        System.out.println(
                "\n-----------------------------------------------" +
                        "\nData " + this.role + " dengan ID KM" + this.getId_magang() + " Berhasil diupdate." +
                        "\n-----------------------------------------------"
        );
    }



}


